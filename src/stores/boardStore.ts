...
import { getElementsMap } from './yjsStore'
...
export const useBoardStore = create<BoardState>((set, get) => ({
  ...
  addElement: (element) => {
    getElementsMap().set(element.id, element)
  ...
  updateElement: (id, changes) => {
    ...
    if (existing) {
      map.set(id, { ...existing, ...changes, updatedAt: new Date().toISOString() })
    }
    set((state) => ({
  ...
  deleteSelectedElements: () => {
    ...
    const map = getElementsMap()
    selectedElementIds.forEach(id => map.delete(id))
    set((state) => ({
  ...
  duplicateSelected: () => {
    ...
    newElements.forEach(el => map.set(el.id, el))
  ...
  syncFromYjs: () => {
    const elements = Array.from(getElementsMap().values())
    set({ elements })
  },
